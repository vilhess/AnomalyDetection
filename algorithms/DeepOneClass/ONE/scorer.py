...
def compute_p_values_test(anomaly_digit):
    ...
    if os.path.isfile(f"models_infos/DeepOneClass/ONE/model_anomaly_{ANOMALY_DIGITS}.pkl"):

        if not os.path.isfile(f"models_infos/DeepOneClass/ONE/p_values_{ANOMALY_DIGITS}.json"):
            ...
            train_dic_dataset, val_dic_dataset, test_dic_dataset = load_datasets(transform=transform)
            ...
            with open(f"models_infos/DeepOneClass/ONE/p_values_{ANOMALY_DIGITS}.json", "w") as file:
                json.dump(final_results, file)

        else:
            with open(f"models_infos/DeepOneClass/ONE/p_values_{ANOMALY_DIGITS}.json", "r") as file:
