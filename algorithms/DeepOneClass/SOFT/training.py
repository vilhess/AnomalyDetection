...
def training(anomaly_digits):
    ...
    for epoch in train_iterator:
        ...
        for imgs in trainloader:
            ...
            if epoch>=WARM_UP_EPOCHS:
                ...
                R = torch.tensor(R).to(torch.float32).to(DEVICE)
    ...
    checkpoint = {
            ...
            "radius":R.detach().cpu().numpy()
        }

    os.makedirs(f'models_infos/DeepOneClass', exist_ok=True)
    os.makedirs(f'models_infos/DeepOneClass/SOFT', exist_ok=True)
        
    
    torch.save(checkpoint, f'models_infos/DeepOneClass/SOFT/model_anomaly_{ANOMALY_DIGITS}.pkl')
